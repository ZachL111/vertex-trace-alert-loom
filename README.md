...
vertex-trace-alert-loom is a Scala project for observability. It focuses on this technical goal: Package a Scala local lab for alert analysis with append-only fixtures, checkpoint recovery checks, and documented operating limits.
...
The core module exposes a small scoring API. Inputs are simple numeric signals: demand, capacity, latency, risk, and weight. The score uses a threshold of 150, risk penalty 4, latency penalty 3, and weight bonus 4. Tests exercise the public API against the fixture cases in `fixtures/cases.csv`.

...
`vertex-trace-alert-loom` is centered on observability work in Scala. The technical target is: Package a Scala local lab for alert analysis with append-only fixtures, checkpoint recovery checks, and documented operating limits.
...
The score is deterministic. The current threshold is 150. Risk has a penalty of 4, latency has a penalty of 3, and weight contributes a bonus of 4.
